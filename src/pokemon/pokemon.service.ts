...
export class PokemonService {
  ...
  findAll(paginationDto: PaginationDto) {
    ...
    return this.pokemonModel.find()
      .limit(limit)
      .skip(offset)//emretoirna el siguiente
      .sort({
        no: 1
      })
      .select("-__v")
  ...
  async update(term: string, updatePokemonDto: UpdatePokemonDto) {
    const pokemon = await this.findOne(term)//busca ya sea por id o por name o por no 
  ...
  private handleExceptions(error: any) {
    ...
    console.log(error); throw new InternalServerErrorException(`Can't  create Pokemon - check server logs`)
